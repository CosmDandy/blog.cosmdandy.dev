---
title: cosmdandy
description: Notes on infrastructure, Kubernetes, CI/CD and everything I break along the way.
---

Инфраструктурные заметки: Kubernetes, Terraform, CI/CD, домашняя лаборатория и то, что ломается по дороге.

Пишу и на русском, и на английском — как ляжет тема.

## Где ещё

- [cosmdandy.dev](https://cosmdandy.dev) — контакты и всё остальное
- [cv.cosmdandy.dev](https://cv.cosmdandy.dev) — резюме
- [GitHub](https://github.com/CosmDandy)

## Последнее

- [[posts/hello|Первая заметка]]
